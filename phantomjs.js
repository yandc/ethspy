...
if (system.args.length !== 2) {
    ...
    var first_response = null,
        ...
        script_result = null;
    var fetch = JSON.parse(system.args[1]);
	...
	if (fetch.headers && fetch.headers['Cookie']) {
		cookies = fetch.headers['Cookie'].split(';');
		var domain = fetch.url.split('/')[2];
		for (x in cookies) {
			var c = cookies[x].split('=');
			var cookie = {'name':c[0], 'value':c[1], 'domain':domain};
			console.log('Cookie: #'+JSON.stringify(cookie, undefined, 4));
			console.log(page.addCookie(cookie));
		}
    ...
    page.onResourceRequested = function(request) {
		console.debug("Starting request: #"+request.id+" ["+request.method+"]"+request.url+"#"+request.postData);
		end_time = null;
    }
    page.onResourceReceived = function(response) {
		//setTimeout("alert('sleep 0.3 second!')",300)
    ...
    // send request
    page.open(fetch.url, {operation:fetch.method, data:fetch.data});
    // make response
    function make_result(page) {
		...
		var path = '/tmp/phantomjs.result.'+fetch.proxy;
		fs.write(path, body, 'w');
		phantom.exit();
		//response.writeHead(200, {'Cache': 'no-cache','Content-Type': 'application/json',});
		//response.write(body);
    ...
    function _make_result(page) {
		...
		var cookies = {};
		page.cookies.forEach(function(e) {cookies[e.name] = e.value;});

		var headers = {};
		if (first_response.headers) {
			first_response.headers.forEach(function(e) {headers[e.name] = e.value;});
		}
